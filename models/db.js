...
const Account= require('./UserModel.js');
const Product = require('./ProductModel.js');
const Log = require('./LoggedModel.js');
const Review = require('./ReviewModel.js');
const Transaction = require('./TransModel.js');
const History = require('./HistoryModel.js');



// ccapdev-mongoose is the name of the database
const url = process.env.MONGOLAB_URI || 'mongodb://localhost:27017/GandaKa';
...
const database = {
    ...
    findManyLimited: function(model, query, projection, limit, callback) {
        model.find(query, projection, function(error, result) {
            if(error) throw error;
            return callback(result);
        }).limit(limit);
    },
    findManyLimitedSorted: function(model, query, projection, sort, limit, callback) {
        model.find(query, projection, function(error, result) {
            if(error) throw error;
            return callback(result);
